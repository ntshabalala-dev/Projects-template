...
(function () {
    ...
    */
    ...
    const sortTodosBy = (todos, sortby = 'due-date') => {
        if (sortby == 'due-date') {
    ...
    */
    ...
    const createTodos = (container, todos) => {
        todos.forEach((todo) => {
            ...
            projectItemDueDate.textContent = todo.dueDate ? format(new Date(todo.dueDate), "d MMM yyyy") : 'No due date';
            log(format(new Date(todo.dueDate), "d MMM yyyy"));
    ...
    */
    ...
    sortBy.addEventListener('change', (e) => {
        ...
        if (showAllTasksDiv.classList.contains('selected')) {
            ...
            const selectedProjectId = document.querySelector('#projects__list button.active').dataset.projectId
            todos = Todos.findAllByProject(selectedProjectId)
        }
        const projectItemsContainer = document.querySelector('.project-items__todos');
        projectItemsContainer.innerHTML = '';

        sortTodosBy(todos, e.target.value);
